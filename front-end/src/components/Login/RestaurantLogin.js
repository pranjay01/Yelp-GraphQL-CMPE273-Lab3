...
import { restaurantSignup, loginUser } from '../../mutations/signupMutations';
...
class RestaurantLogin extends Component {
    ...
    this.props.client
      .mutate({
        ...
        variables: {
          ...
          PhoneNo: Number(this.state.contactNo),
          Role: 'Restaurant',
        },
      })
      .then(
        (response) => {
          ...
          if (response.data.restaurantSignup.Result === 'Restaurant Created') {
          ...
          } else if (response.data.restaurantSignup.Result === 'Email Already Exists') {
            let payload = {
              success: true,
  ...
  submitLogin = (e) => {
    ...
    axios.defaults.withCredentials = true;
    //make a post request with the user data
    // axios.post(serverUrl + 'biz/login', data)
    this.props.client
      .mutate({
        mutation: loginUser,
        variables: {
...
export default compose(
  withApollo,
  graphql(restaurantSignup, { name: 'restaurantSignup' }),
